...
def checkIPs(silkRec,ipValue):
	if ipValue['logic'] == 'none':
		# check wich field was input
		if ipValue['srcaddr'] != None and ipValue['srcaddr'] != str(silkRec.sip):

			info = ("Error: srcaddr",{"query":ipValue['srcaddr'],"rec":str(silkRec.sip)})
	...
	elif ipValue['logic'] == 'and':

		if ipValue['srcaddr'] != str(silkRec.sip) and ipValue['dstaddr'] != str(silkRec.dip):

			info = ("Error: srcaddr AND dstaddr", {"query":(ipValue['dstaddr'],ipValue['srcaddr']),"rec":(str(silkRec.sip),str(silkRec.dip))})

			return False, info

	elif ipValue['logic'] == 'or':

		if ipValue['srcaddr'] != str(silkRec.sip) or ipValue['dstaddr'] != str(silkRec.dip):

			info = ("Error: srcaddr OR dstaddr", {"query":(ipValue['dstaddr'],ipValue['srcaddr']),"rec":(str(silkRec.sip),str(silkRec.dip))})

			return False
	else: 

		info = "Error: ipValue, din't match any logic"

		return False, info

	return True,None

def checkPorts(silkRec,portValue):
	if portValue['logic'] == 'none':

		# check wich field was input
		if portValue['srcport'] != None and portValue['srcport'] != silkRec.sport:

			info = ("Error: srcport",{"query":portValue['srcport'],"rec":silkRec.sport})
	...
	elif portValue['logic'] == 'and':

		# both fields were given, do the and logic
		if portValue['srcport'] != silkRec.sport and portValue['dstport'] != silkRec.dport:

			info = ("Error: srcport AND dstport", {"query":(portValue['dstport'],portValue['srcport']),"rec":(silkRec.sport,silkRec.dport)})

			return False, info

	elif portValue['logic'] == 'or':
		# both fields were given, do the or logic
		if portValue['srcport'] != silkRec.sport or portValue['dstport'] != silkRec.dport:

			info = ("Error: srcport OR dstport", {"query":(portValue['dstport'],portValue['srcport']),"rec":(silkRec.sport,silkRec.dport)})
...
def checkRec(silkRec, queryDic):
	for key in queryDic.keys():
		if key == "ipValue" or key == "portValue":

			# check if ipValues match record
			if key == "ipValue":

				flag, info = checkIPs(silkRec,queryDic[key])

				if not flag: return False, info

			elif key == "portValue":
				flag, info = checkPorts(silkRec,queryDic[key])
